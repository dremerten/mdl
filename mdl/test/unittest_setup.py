...
NOTE!! This script CANNOT import any mdl libraries. Doing so risks
...
CONFIG_SUFFIX = '.mdl_unittests/config'
...
class TestConfig(object):
    ...
    def create(self):
        """Create a good config and set the MDL_CONFIGDIR variable.
...
def _environment():
    ...
    # Make sure the MDL_CONFIGDIR environment variable is set
    if 'MDL_CONFIGDIR' not in os.environ:
        log_message = (
            '\nThe MDL_CONFIGDIR is set to the wrong directory. '
            'Run this command to do so:\n\n'
            '$ export MDL_CONFIGDIR={}'
    ...
    # Make sure the MDL_CONFIGDIR environment variable is set correctly
    if os.environ['MDL_CONFIGDIR'] != CONFIG_DIRECTORY:
        log_message = (
            '\nThe MDL_CONFIGDIR is set to the wrong directory ("{}"). '
            'Run this command to do so:\n\n'
            '$ export MDL_CONFIGDIR={}'
            '\n\nThen run this command again, followed by.\n\n'
            ''.format(
                os.environ['MDL_CONFIGDIR'], CONFIG_DIRECTORY))
        print(log_message)
        sys.exit(2)
